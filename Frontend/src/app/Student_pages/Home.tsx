...
const PeekingKawaiiRight = () => (
  <div className="absolute -right-8 top-12 z-0 w-24 h-32 overflow-visible pointer-events-none">
    ...
    >
      <path d="M 120,20 C 60,20 20,60 20,100 C 20,120 40,140 120,140 Z" fill="#98E2C6" stroke="#76C8A8" strokeWidth="2" />
...
const KawaiiGhost = ({ sizeClass = "w-[120px] h-[120px]" }: { sizeClass?: string }) => (
  <div className={`relative flex items-center justify-center ${sizeClass}`}>
    ...
    >
      <g transform="translate(30, 15)">
        ...
        <circle cx="55" cy="88" r="10" fill="#1E1B4B" /><circle cx="95" cy="88" r="10" fill="#1E1B4B" />
        <circle cx="51" cy="84" r="4" fill="white" /><circle cx="91" cy="84" r="4" fill="white" />
    ...
    <span className="absolute top-2 right-2 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-white" />
          ...
          <div className="mb-8 bg-white p-6 rounded-[34px] shadow-sm flex items-center gap-4 border border-violet-50">
            ...
            <div className="flex-1">
              <h2 className="text-violet-950 text-[26px] font-black leading-none">Сайн уу! 👋</h2>
              <p className="text-violet-900/60 text-[13px] font-bold mt-1">Чиний аюулгүй байдал бидний хамгийн чухал зүйл.</p>
            </div>
          </div>
          ...
          <div className="relative bg-white rounded-[36px] shadow-sm px-5 pt-10 pb-8 mb-7 border border-slate-50">
            ...
            <div className="relative z-10 mx-auto w-[240px] h-[240px] flex items-center justify-center">
              <svg className="absolute inset-0 w-full h-full -rotate-90 pointer-events-none" viewBox="0 0 180 180">
                <circle cx="90" cy="90" r="85" fill="none" stroke="#FFF5F6" strokeWidth="10" />
                <motion.circle
                  cx="90" cy="90" r="85" fill="none" stroke="#FF3B4F" strokeWidth="10" strokeLinecap="round"
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="relative z-10 w-[200px] h-[200px] rounded-full bg-gradient-to-b from-[#FF5F6D] to-[#EF233C] shadow-2xl flex flex-col items-center justify-center text-white touch-none"
              >
                <PhoneCall size={44} className="mb-2" />
                <span className="text-[48px] font-black tracking-tighter leading-none">SOS</span>
              </motion.button>
            </div>
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[92%] max-w-md bg-white/95 backdrop-blur-xl rounded-[34px] shadow-2xl px-6 py-4 z-50 border border-white/50">
        <div className="flex items-center justify-center gap-16">
          <button onClick={() => navigate('/')} className="flex flex-col items-center gap-1 text-violet-700 font-black active:scale-95 transition-transform">
          ...
          <button onClick={() => navigate('/report')} className="flex flex-col items-center gap-1 text-slate-400 font-bold active:scale-95 transition-transform">
            <div className="px-6 py-2.5">
              <HeartHandshake size={26} strokeWidth={2} />
            </div>
            <span className="text-[11px]">Мэдэгдэх</span>
          </button>
        </div>
      </div>
    </div>
